...
final class CloudinaryMediaService implements MediaStorageService {
  ...
  Future<CloudinaryUploadResult> uploadImage({
    ...
    if (decoded is! Map<String, dynamic>) {
      throw const CloudinaryUploadException('Cloudinary returned invalid response');
    }

    final CloudinaryUploadResult result = CloudinaryUploadResult.fromJson(decoded);
    if (result.secureUrl.isEmpty) {
      throw const CloudinaryUploadException('Cloudinary response has empty secure_url');
    }
